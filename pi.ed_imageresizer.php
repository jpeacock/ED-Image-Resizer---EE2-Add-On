 ...
 * This is a fork of the original by Erskine - this version is more portable since it doesn't require that you hard code the server path (it picks it up via $_SERVER['DOCUMENT_ROOT']). Also, it assembles a full url path rather than just a relative path to the cached file. 
 * 
...
Class Ed_imageresizer
{
    ...
    private $cache_path         = 'cache/'; // with trailing slash
    ...
    {
		$this->server_path = substr($_SERVER['DOCUMENT_ROOT'], 0, -1);
		$this->cache_path = $_SERVER['DOCUMENT_ROOT'].$this->cache_path;
		
		//echo substr($_SERVER['DOCUMENT_ROOT'], 0, -1)."<br>";
    ...
    private function _compileImgTag(){
        ...
        $this->image_tag .= 'src="'.$site_url.$image_path.'" />';
